...
class EfficientNetGen(FeatureExtractor):
    def __init__(self, model: str):
        ...
        self.classifier = nn.Linear(
            self.efficientnet._conv_head.out_channels, 1)
...
class EfficientNetB7(EfficientNetGen):
    def __init__(self):
        super(EfficientNetB7, self).__init__(model='efficientnet-b7')
...
class EfficientNetAutoAtt(EfficientNet):
    def init_att(self, model: str, width: int):
        ...
        if model == 'efficientnet-b4':
            ...
            if width == 0:
                self.attconv = nn.Conv2d(
                    kernel_size=1, in_channels=56, out_channels=1)
            else:
                attconv_layers = []
                ...
                attconv_layers.append(('conv_out', nn.Conv2d(
                    kernel_size=1, in_channels=56, out_channels=1)))
